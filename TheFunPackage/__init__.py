...
def version_check():
    ...
    def version_check_2(make_directory):
        ...
        try:
            # Variables
            github_version = json.loads(requests.get("https://api.github.com/repos/anikethchavare/TheFunPackage/releases/latest").text)["tag_name"][1:]

            # Checking the Version Values
            if (github_version > __version__):
                # Checking the Environment
                if ("idlelib.run" in sys.modules):
                    print("You are using TheFunPackage version {0}, however version {1} is available.".format(__version__, github_version))
                    print("Upgrade to the latest version for new features and improvements using this command: pip install git+https://github.com/anikethchavare/TheFunPackage.git" + "\n")
                else:
                    print(Fore.YELLOW + "You are using TheFunPackage version {0}, however version {1} is available.".format(__version__, github_version))
                    print(Fore.YELLOW + "Upgrade to the latest version for new features and improvements using this command: " + Fore.CYAN + "pip install git+https://github.com/anikethchavare/TheFunPackage.git" + Style.RESET_ALL + "\n")
